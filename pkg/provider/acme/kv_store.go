...
// KVStore Stores implementation for key value storage.
type KVStore struct {
...
// NewKVStore initializes a new KVStore with a file name.
func NewKVStore(filename string) *KVStore {
	store := &KVStore{filename: filename, saveDataChan: make(chan map[string]*StoredData)}
	store.listenSaveAction()
	return store
}

func (s *KVStore) save(resolverName string, storedData *StoredData) {
	s.lock.Lock()
	defer s.lock.Unlock()
...
// listenSaveAction listens to a chan to store ACME data in json format into `KVStore.filename`.
func (s *KVStore) listenSaveAction() {
	safe.Go(func() {
		logger := log.WithoutContext().WithField(log.ProviderName, "acme")
...
func (s *KVStore) unSafeCopyOfStoredData() map[string]*StoredData {
